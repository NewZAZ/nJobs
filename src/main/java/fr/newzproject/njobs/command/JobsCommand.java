...
public class JobsCommand extends AbstractCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            Job mineur = manager.getJob(player.getUniqueId(), JobType.MINEUR);
            Job agriculteur = manager.getJob(player.getUniqueId(), JobType.AGRICULTEUR);
            Job chasseur = manager.getJob(player.getUniqueId(), JobType.CHASSEUR);
            ...
            for (int i = 0; i < configuration.getInt("gui.size"); i++) {
                if (configuration.getString("gui.inventory." + i + ".material") != null && configuration.getString("gui.inventory." + i + ".name") != null && configuration.getStringList("gui.inventory." + i + ".lore") == null) {
                    ...
                    configuration.getStringList("gui.inventory." + i + ".lore").forEach(each ->
                            lore.add(ChatColor.translateAlternateColorCodes('&', each.replaceAll("%mineur_level%", String.valueOf(mineur.getLevel())).replaceAll("%mineur_maxLvl%", String.valueOf(10))
                                    ...
                                    .replaceAll("%agriculteur_level%", String.valueOf(agriculteur.getLevel())).replaceAll("%agriculteur_maxLvl%", String.valueOf(10))
                                    ...
                                    .replaceAll("%chasseur_level%", String.valueOf(chasseur.getLevel())).replaceAll("%chasseur_maxLvl%", String.valueOf(10))
